...
<html lang="en">
  ...
  <nav class="navbar navbar-expand-lg bg-white sticky-top">
      ...
      <div class="container py-4">
        <div class="row">
          <div class="col-6 d-flex justify-content-center">
            <img height="600px" id="about-img" src="#" alt="">
          </div>
          <div class="col-6 col-6 d-flex justify-content-center align-items-center">
            <div>
              <p id="about-desc"></p>
              <button class="btn btn-primary rounded-0 mt-3" id="about-button" type="button"></button>
            </div>
          </div>
        </div>
      </div>
    ...
    <section id="location">
      <div class="container py-5">
        <div class="row border p-5" style="--bs-border-width: 7px; --bs-gutter-x: 2rem;">
          <div class="col-6 d-flex justify-content-center">
            <img width="100%" src="<?php echo base_url('resources/img/location-img.png');?>" alt="">
          </div>
          <div class="col-6 col-6 d-flex justify-content-center align-items-center">
            <div>
    ...
    <script>
      //Get Data Our Coffee
      $.get("http://localhost:1337/api/coffees?populate=*&&pagination[limit]=3", function(data, status){
        console.log(data)
        let dataHtml ='';
        data.data.map((e)=>{
        dataHtml += `
      ...
      function moreCoffee(element){
        $(element).hide()
      $.get("http://localhost:1337/api/coffees?populate=*&&pagination[limit]=-1", function(data, status){
      let dataHtml = '';
      ...
      //Get Data Gallery
      $.get("http://localhost:1337/api/galleries?populate=*", function(data, status){
        console.log(data)
        let dataHtml ='';
        data.data.map((e)=>{
        dataHtml += `
      ...
      //Get Data Instagram
      $.get("http://localhost:1337/api/instagrams?populate=*", function(data, status){
        console.log(data)
      ...
      $.get("http://localhost:1337/api/about?populate=*", function(data, status){
        ...
        $("#hero-banner").attr('src','http://localhost:1337'+data.data.attributes.banner.data.attributes.url)
        $("#about-desc").html(data.data.attributes.description)
        $("#about-img")
        .attr('src','http://localhost:1337'+data.data.attributes.picture.data.attributes.url)
        // .attr('width',)
      ...
      $.get("http://localhost:1337/api/setting?populate=*", function(data, status){
       ...
       $("#address").text(data.data.attributes.address)
       $("#opening-hours").text(data.data.attributes.opening_hours)
